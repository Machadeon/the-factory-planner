...
Note: today's real recipe/game data cannot produce this two-key shape via `Factory.createBaseModel` (see the archived `fix-objective-coefficient-typo` change's investigation); this scenario is validated by supplying a model in this shape directly to `autoCalculateRates()`, not by reproducing the shape through normal recipe data.
