...
#### Scenario: R2.S2 — next/image elements respect base path
- **WHEN** the app is built with `NEXT_PUBLIC_BASE_PATH=/the-factory-planner` and a component renders `next/image` with an icon path from the library
- **THEN** the rendered `src` contains `/the-factory-planner` exactly once
