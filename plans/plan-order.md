...
| [ ] | B2.5 | Bug: `splitRecipes` stale `rateLookup` + widen mutation-contract test ([#23](https://github.com/Machadeon/the-factory-planner/issues/23)) | Left open by M4's review, never filed (§6.1 items 2–3). Regression test first per AGENTS.md; add `Factory.splitRecipeRates(pl)` ending in `_updateRates()`, convert the component; widen contract-test regexes to bare `productionLine.`/`assemblyLine.` writes and helper-mediated mutations (`applyRejectSilent(factory.optimizer, …)`). **Must precede B3** — the fix lands in `ProductionLineComponent.tsx` before its rename. |
| [ ] | B3 | [Phase 5 naming/layout sweep](./component-refactor.md#phase-5--naming-layout-and-sweep) | Closes both refactor plans. Fold in logging cleanup (#14) if M2 didn't already delete the commented console lines — same sweep character. Do before any file-touching backlog work so renames don't conflict. **Scope additions from the review:** §4.7 leftovers (`var` ×3 in factory.ts, `useFactoryPageFlows.ts` → `app/hooks/`, stale test filenames incl. `library-ops.test.ts`, `Dividers`/`TextCalculatorField`/`PartSelector` → `ui/`), §4.1 AGENTS.md drift (dead `Factory.update()`/`FactoryComponent` claim; schemaVersion 2 → 1; migration now in `migrations.ts`), §3 dead-code deletions (icons `.js` files, unused exports — reconcile `partLookup`/`buildingLookup` docs-vs-exports deliberately), §6.2 vestigial carryovers (dead `!library` guards in SourceFactoriesEditor, GraphProps optionality, factory.ts double factory-recipe import, `powerPart` decision, reveal-block duplication in ProductionLineDetails), and the last models-layer `as unknown as` at `factory-storage.ts:226` (§4.8 — M3's stated goal was zero). |
