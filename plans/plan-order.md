...
| Done | # | Item | Why here |
| --- | --- | --- | --- |
| [x] | C1 | Styling retrofit per [ADR-0001](./adrs/01-styling-system.md) (A2 decision) | After Phase 5 renames — avoids rebase churn on moved files. **Land the emotion cascade-layer fix first** (`AppRouterCacheProvider enableCssLayer: true` + `@layer` order in `globals.css`); without it every Tailwind override on a retained MUI widget silently no-ops mid-migration. Also resolve here: CssBaseline-vs-preflight, biome `no @mui outside ui/` rule (icons exempt), migrate non-allowlist MUI widgets to Tailwind. **Plus the `m-[-2]` decision** (§6.1 item 4): unitless margin is invalid CSS, silently dropped — restore the −2px overlap intent (`m-[-2px]`, visual change) or bless the shipped look and delete the token. Landed 2026-07-21 via `finish-block-c`: cascade-layer fix + `CssBaseline` removed; biome `noRestrictedImports` rule blocks `@mui/material` outside `ui/` (icons exempt); `m-[-2]`→`m-[-2px]` restored the intended overlap. 13 new/refactored `ui/` primitives (`Button`, `Chip`, `Switch`, `RadioGroup`, `TextField`, `Select`, `Menu`, `Tooltip`, `Dialog`/`Drawer` barrels, `Tabs`, `Slider`, `FactorySelector`; `TextCalculatorField`/`InlineEditText` rebuilt on `TextField`) migrated all ~25 non-`ui/` files off raw MUI. **ADR-0001 amended** to add `Drawer` to the allowlist (focus-trap-adjacent, historical drawer-loop bug risk — see the ADR's added rationale) rather than hand-rolling the library drawer. One real regression caught by e2e (not unit tests): `ui/Switch`'s `sr-only` input let its visual track intercept pointer events; fixed to an `opacity-0`, full-size input under `pointer-events-none` decoration. |
...
| [x] | C3 | [Storage-failure surfacing (#4)](./codebase-improvements.md#4-silent-storage-failure--data-loss) + [ErrorBoundary (#5)](./codebase-improvements.md#5-react-errorboundary) | Data-loss protection; needs toast (C2) and benefits from M5's clean storage layer. Batch as one "resilience" change. Landed 2026-07-21 via `finish-block-c`: `saveLibrary`/`writeAutosave` return `boolean` instead of swallowing quota errors; `useLibrary`/`useFactorySession`/`useAutosave` surface failures as error toasts (autosave's teardown-path flushes stay silent — a toast can't paint mid-unload) and leave the dirty badge lit on failure; one-shot quota-approaching `info` toast at 90% of 5MB. New `ui/ErrorBoundary.tsx` class component wraps `FactoryPage` in `page.tsx` with a "Try again"/"Export your data" recovery screen. Closed the C2 follow-up: `ToastProvider`'s queue is now capped at 20, dropping the oldest entry. |
| [x] | C4 | [RateDisplay non-color signaling (#12)](./codebase-improvements.md#12-non-color-status-signaling--a11y-rules) | One-file change now that RateDisplay exists. a11y biome rules already active per AGENTS.md — just the icon/text affordance remains. **Plus keyboard-accessible sidebar splitter** (§6.1 item 5): `FactorySidebar.tsx`'s biome-ignore claims it's "tracked separately" — this is that tracking. Last Clickable-era a11y hole. Landed 2026-07-21 via `finish-block-c`: `RateDisplay` gained a `status` prop (surplus/deficit/balanced/slooped) rendering an `aria-hidden` icon alongside the existing color class; `ProductionLineRow`/`Recipe` now render through it instead of inlining color logic. Splitter is now a keyboard-operable `<hr>` (`role` implicit via the element, `aria-orientation`/`aria-valuenow/min/max`, arrow-key resize via a new `handleResizeKeyDown` in `useDragResize`); the stale biome-ignore is gone. |
| [x] | C5 | [Game-data versioning doc (#15)](./codebase-improvements.md#15-game-data-versioning) | Trivial filler, slot anywhere. Landed 2026-07-21 via `finish-block-c`: `docs/game-data.md` records current contents (175 items/276 recipes/etc, ~825KB), and honestly documents that the game version and generation source are unrecoverable from git history rather than inventing a provenance chain. Also closed the `github-pages-deploy` residual (§6.3): a live click-through against the deployed site (add product → recipe selection → cascading rates → reload → consent → library drawer, zero console errors) formally verified task 5.4; task 5.5 (workflow-dispatch trigger test) was left open as a state-changing action outside a read-only verification pass. |
