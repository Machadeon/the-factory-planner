...
- Audit all model interfaces and how their fields and methods are used. Ensure proper interfaces (e.g. no protected member access). Applies to **every** model in the repo — `Factory`, `ProductionLine`, `AssemblyLine`, `Recipe`/`FactoryRecipe`, `Part`, `Building` — not just `Factory`.
